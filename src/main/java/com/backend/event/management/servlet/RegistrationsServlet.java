	...
	private void findByEventId(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			int eventId = Integer.parseInt(request.getParameter("eventId"));
             List<RegistrationsDTO> registrationsDTOList = registrationsService.findByEventId(eventId);
	...
	private void findByUserId(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			int userId = Integer.parseInt(request.getParameter("userId"));
             List<RegistrationsDTO> registrationDTOList = registrationsService.findByUserId(userId);
			...
			if ("XMLHttpRequest".equals(isAjax)) {
				...
				response.setCharacterEncoding("UTF-8");
				Gson gson = new Gson();
				response.getWriter().write(gson.toJson(registrationDTOList));
				return;
			}
