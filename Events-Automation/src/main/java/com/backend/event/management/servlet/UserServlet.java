...
//import com.backend.event.management.util.EmailUtil;
...
//			}else if("saveResetToken".equalsIgnoreCase(task)) {
//				String email = request.getParameter("email");
//				UserDTO user = userService.findByEmail(email);
//				if (user == null) {
//					response.sendRedirect("forget-reset-verification-password.jsp?msg=Email not registered");
//					return;
//				}
//				String token = java.util.UUID.randomUUID().toString();
//				Timestamp expiry = new Timestamp(System.currentTimeMillis() + (15 * 60 * 1000));
//				userService.saveResetToken(email, token, expiry);
//
//				String resetLink = "http://yourdomain/forget-reset-verification-password.jsp?token=" + token;
//				EmailUtil.sendEmail(email, "Password Reset Link",
//						"Click this link to reset your password: " + resetLink);
//
//				response.sendRedirect("forget-reset-verification-password.jsp?msg=Reset link sent to your email");
//
//			} else if ("resetPassword".equalsIgnoreCase(task)) {
//				String token = request.getParameter("token");
//				String newPassword = request.getParameter("newPassword");
//				UserDTO userDTO = userService.findByResetToken(token);
//				if (userDTO == null) {
//					response.sendRedirect("forget-reset-verification-password.jsp?msg=Invalid or expired token");
//					return;
//				}
//				userService.resetPassword(userDTO.getId(), newPassword);
//				userService.clearVerificationToken(userDTO.getId());
