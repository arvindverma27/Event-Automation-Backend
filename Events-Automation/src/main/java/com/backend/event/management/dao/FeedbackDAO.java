...
//	private final String FIND_FEEDBACK_BY_EVENTS_NAME = "select  feedback.id, feedback.events_id, events.event_name AS event_name,feedback.users_id,  users.full_name AS full_name, feedback.rating, feedback.comments, feedback.feedback_date FROM feedback"
//			+ "JOIN events ON feedback.events_id = events.id " + "JOIN users ON feedback.users_id = users.id"
//			+ "WHERE feedback.event_name = ?";
...
//			connection = dbUtil.getConnection();
//
//			pstmt = connection.prepareStatement(FIND_FEEDBACK_BY_EVENTS_NAME);
//
